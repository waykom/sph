...
export default {
  ...
  methods: {
    async getPayInfo() {
      const res = await this.$API.PayInfoAPI(this.$route.query.orderId);
      if (res.code === 200) {
    ...
    async show() {
      ...
      this.$alert(`<img src="${res}" style="width:280px">`, {
        dangerouslyUseHTMLString: true,
        lockScroll: false,
        center: true,
        showCancelButton: true,
        cancelButtonText: "支付遇到问题？",
        confirmButtonText: "订单已支付",
        showClose: false,
      ...
      if(!this.timer) {
        this.timer = setInterval(async () => {
          const res = await this.$API.PayStatusAPI(this.payInfo.orderId)
          if(res.code === 200) {
            clearInterval(this.timer)
            this.timer = null
            this.code = res.code
